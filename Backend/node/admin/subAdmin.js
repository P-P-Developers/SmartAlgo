module.exports = function (app, connection1) {
    ...
    app.post("/admin/subadmin/add", (req, res) => {
        ...
        var api_update = req.body.api_update;
        var SubAdmin_strategy = req.body.store_strategy


        // console.log("SubAdmin_strategy", SubAdmin_strategy);
    ...
    app.post("/admin/subadmin/update", (req, res) => {
        ...
        var api_update = req.body.api_update;
        var SubAdmin_strategy = req.body.store_strategy
        var clientnumber_permission = req.body.clientnumber_permission;
        var twodays_demo = req.body.twodays_demo;
        var live = req.body.live;
    
        // const roleId = 4;
        var new_startegy = []

        console.log('UPDATE  `tbl_users` SET `email`="' + email + '",`name`="' + name + '",`mobile`="' + mobile + '",`updatedBy`="' + adminId + '",`updatedDtm`="' + ccdate + '", `add__edit_live`="' + add_live + '", `editClient_live`="' + edit_live + '", `goto_dashboard`="' + goto_dashboard + '", `license_permission`="' + license_permission + '", `group_permission`="' + group_permission + '", `strategy_permission`="' + strategy_permission + '" ,`groupServicesPermission`="'+groupServicesPermission+'" ,`api_update`="'+api_update+'",`clientnumber_permission`="'+clientnumber_permission+'" ,`twodays_demo`="'+twodays_demo+'" ,`live`="'+live+'"  WHERE `userId`="' + subAdminId + '"');

   


        connection1.query('UPDATE  `tbl_users` SET `email`="' + email + '",`name`="' + name + '",`mobile`="' + mobile + '",`updatedBy`="' + adminId + '",`updatedDtm`="' + ccdate + '", `add__edit_live`="' + add_live + '", `editClient_live`="' + edit_live + '", `goto_dashboard`="' + goto_dashboard + '", `license_permission`="' + license_permission + '", `group_permission`="' + group_permission + '", `strategy_permission`="' + strategy_permission + '" ,`groupServicesPermission`="'+groupServicesPermission+'" ,`api_update`="'+api_update+'",`clientnumber_permission`="'+clientnumber_permission+'" ,`twodays_demo`="'+twodays_demo+'" ,`live`="'+live+'"  WHERE `userId`="' + subAdminId + '"', (err, result) => {
            console.log("err", err)
    ...
    // app.get("/admin/subadmin/bookdeliverynew", function(req, res) {
    //     console.log('===========================================');
    //     const pdfPathName = "./admin/generatedfile.pdf";
    //     // FileDownload(pdfPathName, 'file.txt');
    //     // res.download(pdfPathName, "bvhgv.pdf");
    //     // res.download(pdfPathName, (err) => {
    //     //     if (err) console.log(err);
    //     // });

    //     console.log("Pdf path is", __dirname + '/generatedfile.pdf');
    //     res.contentType("application/pdf");
    //     res.status(200).sendFile(__dirname + '/generatedfile.pdf');
    //     console.log('===========================================');
    // });
