module.exports = function (app, connection1) {
    ...
    app.post("/dawnload/data", async (req, res) => {
        ...
        try {
            ...
            if (key == 1) {
                query = `
                  SELECT client.username, client.full_name, trading_status_client.trading, trading_status_client.created_at
                  FROM trading_status_client
                  JOIN client ON client.id = trading_status_client.client_id
                  WHERE trading_status_client.client_id = ? AND trading_status_client.trading != ""
                `;
            } else if (key == 2) {
                query = `
                  SELECT client.username, client.full_name, broker_response.symbol, broker_response.order_id, broker_response.order_status, 
                         broker_response.reject_reason, broker_response.trading_status, broker_response.broker_enter, 
                         broker_response.open_possition_qty, broker_response.created_at
                  FROM broker_response
                  JOIN client ON client.id = broker_response.client_id
                  WHERE broker_response.client_id = ?
                `;
            }
    ...
    app.post("/admin/client/update", verifyToken, (req, res) => {
        ...
        // console.log("req.body.showServiceMonthGiven", req.body.showServiceMonthGiven);
        ...
        const getEndDate = (month) => {
            ...
            let end_date_live_month = new Date(CurrentDate.setMonth(CurrentDate.getMonth() + +month))
        ...
        // connection1.query('SELECT * FROM `tbl_users` WHERE userId = "'+req.body.admin_id+'"', (err, admin_info) => {

        //     var EditClientLive = admin_info[0].license_permission
                ...
                connection1.query('SELECT * FROM `client` WHERE `id` = "' + useradata.id + '"', (err, db_exist_service) => {
                        ...
                        connection1.query('SELECT * FROM `service_and_group_id` WHERE service_group_id=' + req.body.group, (err, resultGroup) => {
                            ...
                            if (add_service.length > 0) {
                                add_service.forEach(function (item, index) {
                                    ...
                                    connection1.query('SELECT `client_id`,`service_id` FROM `client_service` WHERE `client_id` = "' + id + '" AND `service_id` = "' + item + '"', (err, insert_service_and_client_id) => {
                                        ...
                                        if (insert_service_and_client_id.length == 0) {
                                           // console.log("item",item);
                                            ...
                                            connection1.query('INSERT INTO `client_service`  (`service_id`,`client_id`,`service_group_id`,`created_at`,`strategy`,`unique_c_s`) VALUES ("' + item + '",' + id + ',"' + group_id + '","' + dformat + '","' + req.body.strategy[0] + '","' + id + '_' + item + '")', (err, result) => {
                                                console.log(err);
                                                //  console.log(result);
                            ...
                            if (add_service.length > 0) {
                                connection1.query('SELECT * FROM `service_and_group_id` WHERE `service_group_id` =' + req.body.group, (err, serviseID) => {
                                    serviseID.forEach((Val) => {
                                        connection1.query('UPDATE `client_service` SET `qty`= "' + Val.group_qty + '" WHERE `client_id`=' + id + ' AND `service_group_id` = ' + Val.service_group_id + ' AND `service_id`=' + Val.service_id, (err, updateQty) => {
                    ...
                    connection1.query('SELECT `strategy_client`.* FROM `strategy_client` LEFT JOIN `client` ON `client`.`id` = `strategy_client`.`client_id` WHERE `strategy_client`.`client_id` = ' + id, (err, dss) => {
                        ...
                        if (add_startegy.length > 0) {
                            ...
                            if (role_id != 4) {
                                connection1.query('INSERT INTO `trading_status_client` (`client_id`,`strategy`,`trading`,`created_at`,`user_status`) VALUES ("' + useradata.id + '","' + add_startegy + '"," ADD STRATEGY ","' + ccdate + '","1")', (err, result) => { });
                            } else {
                                connection1.query('INSERT INTO `trading_status_client` (`client_id`,`strategy`,`trading`,`created_at`,`user_status`) VALUES ("' + useradata.id + '","' + add_startegy + '"," ADD STRATEGY ","' + ccdate + '","4")', (err, result) => { });
                            }

                        }

                        var replace_strategy = [];
                        if (delete_startegy.length > 0) {
                            // console.log("ok", delete_startegy);
                            if (role_id != 4) {
                                connection1.query('INSERT INTO `trading_status_client` (`client_id`,`strategy`,`trading`,`created_at`,`user_status`) VALUES ("' + useradata.id + '","' + delete_startegy + '"," REMOVE STRATEGY ","' + ccdate + '","1")', (err, result) => { });
                            } else {
                                connection1.query('INSERT INTO `trading_status_client` (`client_id`,`strategy`,`trading`,`created_at`,`user_status`) VALUES ("' + useradata.id + '","' + delete_startegy + '"," REMOVE STRATEGY ","' + ccdate + '","4")', (err, result) => { });
                            }
                            ...
                            delete_startegy.forEach(function (item5, inedx) {
                                ...
                                connection1.query('SELECT * FROM `strategy_client` WHERE `client_id`=' + id + ' ORDER BY id DESC', (err, stgfind) => {
                                    ...
                                    connection1.query('SELECT * FROM `client_service` WHERE `client_id`= ' + id, (err, result) => {
                                        // console.log("result", result);


                                        result.forEach(function (item6, inedx) {
                                            // connection1.query('UPDATE `client_service` SET  `strategy`= "' + stgfind[0].strategy + '" WHERE  `id` ="' + item6.id + '" AND `strategy`= "' + item5 + '" AND `strategy`= "undefined"', (err, result2) => {
                                                connection1.query('UPDATE `client_service` SET  `strategy`= "' + stgfind[0].strategy + '" WHERE  `id` ="' + item6.id + '"', (err, result2) => {

                                                if (replace_strategy.length == 0) {
                                                    replace_strategy.push(1);
                                                    var msg_update_startegy = item5 + " ...Removed And " + stgfind[0].strategy + " Updatd. ";
                                                    if (role_id != 4) {
                                                        connection1.query('INSERT INTO `trading_status_client` (`client_id`,`strategy`,`trading`,`created_at`,`user_status`) VALUES ("' + useradata.id + '","' + msg_update_startegy + '","","' + ccdate + '","1")', (err, result) => { });

                                                    } else {
                                                        connection1.query('INSERT INTO `trading_status_client` (`client_id`,`strategy`,`trading`,`created_at`,`user_status`) VALUES ("' + useradata.id + '","' + msg_update_startegy + '","","' + ccdate + '","4")', (err, result) => { });
                                                    }
                    ...
                    if (admin_licence < all_client_licence) {
                        ...
                        var DurationTime;
                        if (req.body.showServiceMonthGiven == "") {
                            DurationTime = 0
                        } else {
                            DurationTime = req.body.showServiceMonthGiven
                        }
                        ...
                        //console.log('else');
                        connection1.query('UPDATE `client` SET `full_name`="' + useradata.full_name + '",`mobile`="' + useradata.mobile + '",`created_at`="' + created_at + '",`start_date`="' + start_date + '",`end_date`="' + end_date + '",`broker`="' + useradata.broker + '",`api_key`="' + useradata.api_key + '",`api_secret`="' + useradata.api_secret + '",`to_month`="' + to_month + '",`licence_type`="' + useradata.licence_type + '",`app_id`="' + useradata.app_id + '",`client_code`="' + useradata.client_code + '",`api_type`="' + useradata.api_type + '",`subadmin_id`="' + subadmin_id + '",`admin_id`="' + admin_id + '",`status`="' + status + '",`status_term`="' + status_term + '",`subadmin_client_status`="' + subadmin_client_status + '", demat_userid="' + useradata.demat_userid + '", duration="' + DurationTime + '" WHERE `id`=' + useradata.id, (err, result) => {
                            // console.log("result", result);
                            ...
                            if (db_exist_service[0].broker != useradata.broker) {
                                ...
                                connection1.query('UPDATE `client` SET `access_token`="",`trading_type`="off" WHERE id=' + useradata.id, (err, result) => { })
    ...
    app.post("/admin/client/add", verifyToken, (req, res) => {
        ...
        connection1.query('SELECT licence FROM `tbl_users` WHERE userId = "1"', async (err, exist_admin_licence) => {
            ...
            connection1.query('SELECT SUM(to_month) as count_exist_licence FROM `client` WHERE `status_term` = "1"', async (err, exist_client_month_count) => {
                    ...
                    connection1.query('SELECT * FROM `client` WHERE `username` = "' + useradata.user_name + '"', (err, username_result) => {
                        connection1.query('SELECT * FROM `client` WHERE `email` = "' + useradata.email + '"', (err, email_result) => {
                            ...
                            if (username_result.length > 0) {
                                ...
                                connection1.query('INSERT INTO `client`  (`full_name`,`email`,`mobile`,`password`,`new_password`,`username`,`created_at`,`start_date`,`end_date`,`broker`,`api_key`,`api_secret`,`to_month`,`licence_type`,`app_id`,`client_code`,`api_type`,`admin_id`,`subadmin_id`,`status`,`status_term`,`subadmin_client_status`,`client_key`,`demat_userid`,`duration`) VALUES ("' + useradata.full_name + '","' + useradata.email + '","' + useradata.mobile + '","' + rand_password + '","' + ByCryptrand_password + '","' + useradata.user_name + '","' + ccdate + '","' + start_date + '","' + end_date + '","' + broker + '","' + useradata.api_key + '","' + useradata.api_secret + '","' + to_month + '","' + useradata.licence_type + '","' + useradata.app_id + '","' + useradata.client_code + '","' + useradata.api_type + '","' + admin_id + '","' + subadmin_id + '","' + status + '","' + status_term + '","' + subadmin_client_status + '","' + client_key + '","' + useradata.demat_userid + '", "' + DurationTime + '")', (err11, result) => {
                                    ...
                                    connection1.query('INSERT INTO `client_group_service`  (`client_id`,`group_id`) VALUES ("' + client_id + '","' + group_id + '")', (err, result) => {
                                        ...
                                        connection1.query('SELECT *  FROM `service_and_group_id` WHERE `service_group_id` =' + group_id, (err, GroupServicesQtyId) => {

                                            GroupServicesQtyId.forEach(function (item, index) {

                                                connection1.query('SELECT `client_id`,`service_id` FROM `client_service` WHERE `client_id` = "' + client_id + '" AND `service_id` = "' + item.service_id + '"', (err, insert_service_and_client_id) => {
                                                    ...
                                                    if (insert_service_and_client_id.length == 0) {
                                                        ...
                                                        connection1.query('INSERT INTO `client_service`  (`service_id`,`client_id`,`service_group_id`,`qty`,`created_at`,`strategy`,`unique_c_s`) VALUES ("' + item.service_id + '",' + client_id + ',"' + group_id + '","' + item.group_qty + '","' + dformat + '","' + strategy_first + '","' + client_id + '_' + item.service_id + '")', (err, result) => {
                                                            console.log(err);
                                        ...
                                        //     connection1.query('SELECT `client_id`,`service_id` FROM `client_service` WHERE `client_id` = "' + client_id + '" AND `service_id` = "' + item + '"', (err, insert_service_and_client_id) => {
                                        //         console.log("insert_service_and_client_id", insert_service_and_client_id.length);

                                        //         if (insert_service_and_client_id.length == 0) {


                                        //             connection1.query('INSERT INTO `client_service`  (`service_id`,`client_id`,`service_group_id`,`created_at`,`strategy`) VALUES ("' + item + '",' + client_id + ',"' + group_id + '","' + dformat + '","' + strategy_first + '")', (err, result) => {
                                        //                 console.log(err);
                                        //                 //  console.log(result);
    ...
    app.post("/admin/client/changestatus", verifyToken, (req, res) => {
        ...
        if (status_term == "1") {
            ...
            if (status == "1") {
                connection1.query('INSERT INTO `trading_status_client` (`client_id`,`strategy`,`created_at`) VALUES ("' + id + '"," ACTIVE ","' + ccdate + '")', (err, result) => { });

            } else {
                connection1.query('INSERT INTO `trading_status_client` (`client_id`,`strategy`,`trading`,`created_at`) VALUES ("' + id + '","IN_ACTIVE ","TradingOFF","' + ccdate + '")', (err, result) => {

                    connection1.query('UPDATE `client` SET `access_token`="" , `trading_type`="off",`login_status`="' + 0 + '" WHERE `id`=' + id, (err, result) => {
                        // console.log("err", err);
                    })
    ...
    app.get("/admin/client-dashboard/:id", (req, res) => {
        ...
        connection1.query('SELECT * from client where `id`="' + id + '"', async (err, result) => {
            ...
            if (result.length !== 0) {
                ...
                var msg = { 'name': result[0].username, 'full_name': result[0].full_name, 'user_id': result[0].id, 'token': token, 'is_term': result[0].is_term, 'mobile': result[0].mobile, 'status': result[0].status, 'expiry': end_date, "expiry_status": result[0].expiry_status, "login_status": result[0].login_status };
