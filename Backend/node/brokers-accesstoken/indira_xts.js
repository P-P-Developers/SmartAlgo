...
module.exports = function (app, connection1) {
  ...
  app.post("/indira_xts/access_token", (req, res) => {
    // console.log("indira_xts");
    ...
    connection1.query('SELECT * from client where `id`="' + user_id + '"', (err, result) => {
      ...
      var config = {
        method: 'post',
        url: 'https://xts.indiratrade.com/interactive/user/session',
        headers: {
          'Content-Type': 'application/json'
        },
        data: data,
        httpsAgent: httpsAgent // Pass the custom HTTPS agent to the Axios configuration
      };
