...
function Tradehistory() {
  ...
  // const [notMatchLE, setNotMatchLE] = useState("")
  ...
  useEffect(() => {
    ...
    currentDate1.setDate(currentDate1.getDate() - 8);
    ...
    axios({
        ...
        lxType.forEach(function (item1) {
          ...
          if (!indexArray.includes(item1.id)) {
            ...
            if (!indexArray.includes(item.id)) {
              ...
              if (item.trade_symbol == item1.trade_symbol && item.segment == item1.segment && item.strategy_tag == item1.strategy_tag && new Date(item.created_at) < new Date(item1.created_at)) {

               // console.log("if", item.created_at);
               // console.log("if 2", item1.created_at);
      ...
      console.log("arrayfind1", arrayfind1);
      var token_string = [];

      arrayfind1.forEach((sig, i) => {

        if (sig.segment == 'FO' && sig.option_type == 'PUT') {
          if (sig.type == 'LE') {

            if (signals[i + 1] != undefined) {
              if (signals[i + 1].type == 'LX' && new Date(signals[i + 1].created_at) > new Date(sig.created_at) && signals[i + 1].trade_symbol == sig.trade_symbol) {
                // return '';
              } else {
                if (!token_string.includes(sig.exchange_symbol + '|' + sig.token + '#')) {
            ...
            if (signals[i + 1] != undefined) {
              //console.log("signal if");
              if (signals[i + 1].type == 'SX' && new Date(signals[i + 1].created_at) > new Date(sig.created_at) && signals[i + 1].trade_symbol == sig.trade_symbol) {
                //  return '';
              } else {
                if (!token_string.includes(sig.exchange_symbol + '|' + sig.token + '#')) {
                  token_string.push(sig.exchange_symbol + '|' + sig.token + '#');
      ...
      //  console.log("token_string ss", token_string.join(',').replace(/,/g, ''));
      let tokenStr = token_string.join(',').replace(/,/g, '');
      //  console.log("final string -- ",tokenStr.substring(0, tokenStr.length - 1));
    ...
    function tokenStringReg(token) {
        ...
        if (sockets == null) {
          runSocket(token);
          var userid = "616587";
          var usersession = "";
        ...
        axios(config)
          .then(function (response) {
            ...
            if (response.data.status == "true") {
              ...
              createSession(response.data.data[0].client_code, response.data.data[0].access_token, token);
            } else {
      ...
      function connect(userId, userSession, token) {
        ...
        socket.onmessage = function (msg) {
          ...
          if (response.tk) {
           ...
           // console.log("live_data" + response.tk)
           ...
           const pl_qty = $('.get_qty'+response.tk).html();
            ...
            if(pl_live != undefined){
             ...
             if(response.bp1 != undefined){
              ...
              const live_pl =( parseFloat(response.bp1) - parseFloat(price))*pl_qty;
              ...
              if(response.sp1 != undefined){
                ...
                const live_pl = (parseFloat(price) -parseFloat(response.sp1))*pl_qty;
  ...
  //        if(signals[i - 1].type == 'LE' && new Date(signals[i - 1].created_at) < new Date(sig.created_at)){
  ...
  //       if(signals[i - 1].type == 'SE' && new Date(signals[i - 1].created_at) < new Date(sig.created_at)){
  //         pl =  (( sig.price - prev_price) * (quantity)).toFixed(2);
  //          }else{
  //          pl = 0;
  //          }
  ...
  const filterSymbols = (e) => {
    var filterSymbols = searchFilter.filter((filterdata) => {
      if ((filterdata.trade_symbol && filterdata.trade_symbol.toLowerCase().toString().includes(e.toLowerCase())) || (filterdata.strategy_tag && filterdata.strategy_tag.toLowerCase().toString().includes(e.toLowerCase()))
      ) {
        return filterdata
      }
    })
  ...
  const token_return_entry_trade = (sig, i) => {
    ...
    if (sig.segment == 'FO' && sig.option_type == 'PUT') {
      ...
      if (sig.type == 'LE') {
       ...
       // console.log("sig.type LE", sig.type);

        if (signals[i + 1] != undefined) {
          if (signals[i + 1].type == 'LX' && new Date(signals[i + 1].created_at) > new Date(sig.created_at) && signals[i + 1].trade_symbol == sig.trade_symbol) {
            return '';
          } else {
           // console.log("sig.type LE inside ", sig.token);
            return sig.token
          }
        } else {
          //console.log("sig.type LE inside undefine else ", sig.token);
          return sig.token
        }
...
// console.log("selectedRows",selectedRows);
  ...
  const handleSelectAll = (event) => {
    ...
    if (checked) {
      setSelectedRows(signals.map((row,i) =>{ 
  ...
  const getSelectedRowsData = () => {
   ...
   if (selectedData.length == 0) {
     ...
     console.log("selectedData last", selectedData)
     ...
     console.log("signal id",signal_id);
      var request = "id:"+signal_id+"@@@@@input_symbol:" + row.symbol + "@@@@@type:" + type + "@@@@@price:" + price + "@@@@@dt:1668504000@@@@@qty_percent:100@@@@@order_type:Market@@@@@client_key:" + panelKey + "@@@@@exchange:"+row.exchange_symbol+"@@@@@product_type:MIS@@@@@strike:" + row.strike + "@@@@@segment:"+row.segment+"@@@@@option_type:" + row.option_type + "@@@@@expiry:" + row.expiry + "@@@@@strategy:" + row.strategy_tag + "@@@@@sq_value:00.00@@@@@sl_value:00.00@@@@@tr_price:00.00@@@@@tsl:00.00@@@@@demo:demo";
          ...
          <div className="col-md-12">
            <div className="card">
              ...
              <div className="col-md-12 text-end">
                ...
                <h6 className="text-center mt-3">This results is valid for today only, We do not directly or indirectly make any reference to the past or expected future return/performance of the Algorithm.
                  <br />
                  <br />
                </h6>
              ...
              <div className="card-body">
                <div className="row trade-space">
                  <div className="col-md-2">
                    ...
                    <input type="number" value={quantity} onChange={(e) => { quantitychange(e) }} min="1" className="form-control" />
                  </div>
...
{/* 
                  ...
                  <div className="col-md-2">
                    <button className="btn btn-color" onClick={() => reset()}>Reset</button>
                  </div>
                ...
                <button className="btn btn-color" onClick={getSelectedRowsData}>Square Off All</button>


                <div className="table-responsive tableheight thistory-dark-font" >
                  <table className="table tbl-tradehostory" id="table" style={
                    // locationname === '180.149.241.128:3000' ?
                    {
                      backgroundImage: `url(/images/${companyDetails && companyDetails[0].tradehistory_watermark})`,
                    ...
                    <thead className="text-dark thistory-dark-font" style={{ fontWeight: 'bold', color: 'white' }}>
                      <tr>
                        <th className="tradehostory-w" style={{ fontWeight: 'bold', color: 'black', width: '100px' }} >
                           
                           {signals.length != 0 ? <input type="checkbox" checked={selectedRows.length === signals.length} onChange={handleSelectAll} />: ""}           
                        ...
                        <th style={{ fontWeight: '900', color: 'black !important', width: '200px', }}>
                          Signal Time
                        </th>
                        <th style={{ fontWeight: '900', color: 'black', width: '250px' }}>
                          Symbol
                        </th>
                        <th style={{ fontWeight: '900', color: 'black', width: '100px' }}>
                          STRAT
                        </th>
                        <th style={{ fontWeight: '900', color: 'black', width: '150px' }}>
                          Type
                        </th>
                        <th style={{ fontWeight: '900', color: 'black', width: '150px' }}>
                          Quantity
                        ...
                        <th style={{ fontWeight: '900', color: 'black', width: '220px' }}>
                          Cumulative P & L
                        </th>
                      </tr></thead>
                    <tbody>
                      ...
                      // console.log("trade symbol", sig.trade_symbol)
                        ...
                        return (
                          <>
                            <tr key={i}>
                             {
                              sig.type == 'LE' || sig.type == 'SE' ?   token_return_entry_trade(sig, i) !== ""  ? 
                              ...
                              <td> 
                              {
                                  ...
                                  onChange={(event) => handleRowSelect(event, sig.id)}
                                 />
                                   :"" : ""}
                              ...
                              <td>{sig.trade_symbol}<b>[{sig.segment}]</b></td>
                              <td>{sig.strategy_tag}</td>
                              <td>{sig.type}</td>
                              <td>{quantity}</td>
                              <td>{(sig.type == 'LE' || sig.type == 'SE' ? sig.price : '-')}</td>
                              <td>{sig.type == 'LX' || sig.type == 'SX' ? sig.price : '-'}</td>
                            
                              <td style={{ display: 'none' }} className={"sp1_price" + token_return_entry_trade(sig, i)}></td>

                              <td style={{ display: 'none' }} className={"bp1_price" + token_return_entry_trade(sig, i)}></td>


                             <td style={{ display: 'none' }} className={"get_qty" + token_return_entry_trade(sig, i)}>{quantity}</td>
                          
                            <td style={{ display: 'none' }} className={"get_pl_live" + token_return_entry_trade(sig, i)}>{sig.price+sig.type}</td>
                            
                              <td  style={{ color: ProfitLossColor(sig, i), fontWeight: 'bold' }}>
                                {
                              ...
                              <td className={"cpl_single" + token_return_entry_trade(sig, i)}  style={{ color: CommuLativeProfitLossColor(cpl, i, trade_array, pl), fontWeight: 'bold' }} > {
                            ...
                            {(locationname == 'software.adonomist.com') &&
                              <tr>
                                <td colspan="3"></td>
                                <td colspan="7">{sig.type == 'LX' || sig.type == 'SX' ? <span>Trade history is not actual profit and loss</span> : ''}</td>
                              </tr>}
                          </>
                        )
                        //   }
                    ...
                    {<tfoot>
                      <tr>
                        <td></td>
                        <td colspan="9" style={{ fontWeight: 'bold', fontSize: '15px' }}>
                          Total cumulative P&L
                        </td>
                        <td className={"cpl"} style={{ color: totalCPL(tcpl, cpl), fontWeight: 'bold', fontSize: '15px' }}>
