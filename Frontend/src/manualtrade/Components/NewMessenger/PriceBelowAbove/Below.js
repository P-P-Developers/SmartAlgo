...
const Below = () => {
  ...
  const [stretegydata, SetStretegydata] = useState('')
    ...
    const AdminPanelKeyRef = useRef("");
  
    
    console.log("clientSegment - ",clientSegment);
    
    console.log("AdminPanelKeyRef - ",AdminPanelKeyRef.current);
    ...
    const getPanelKey = () => {
      ...
      axios(config)
        .then(function (response) {
          ...
          AdminPanelKeyRef.current = response.data.PanelKey[0].panel_key
  ...
  const onAlertClose = (e) => {
   ...
   const containerStyle = {
    ...
    const columns = [
        ...
        {
          name: <h6>By Trade</h6>,
          selector: (row, index) => AdminPanelKeyRef.current == row.panelKey ? "ADMIN" : row.panelKey,
          width: '100px !important',
       },
        {
