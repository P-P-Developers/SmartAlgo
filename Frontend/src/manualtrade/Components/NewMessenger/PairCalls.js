...
const PairCalls = () => {
  ...
  const handleAddRow = () => {
    const newRow = { id: rows.length + 1, script_type: `Row ${rows.length + 1}`, script_name: `Row ${rows.length + 1}`, lot_type: `Row ${rows.length + 1}`, numberof_lot: `Row ${rows.length + 1}`, type: `Row ${rows.length + 1}`, entry_price: `Row ${rows.length + 1}`, };
